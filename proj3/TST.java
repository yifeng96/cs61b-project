...
public class TST {
    ...
    public ArrayList<String> search(String prefix, int k) {
        ...
        BComparator comparato2 = new BComparator();
        PriorityQueue<Node> pq = new PriorityQueue<Node>(1, comparator);
        PriorityQueue<Node> minpq = new PriorityQueue<Node>(1, comparato2);
        Node x = get(root, prefix, 0);
        if (x == null) {
            return queue;
        }
    ...
    public void collect(Node x, StringBuilder prefix, ArrayList<String> queu) {
        if (x == null) {
            return;
        }
        collect(x.left, prefix, queu);
        if (x.val != null) {
            queu.add(prefix.toString() + x.c);
        }
        collect(x.mid, prefix.append(x.c), queu);
        prefix.deleteCharAt(prefix.length() - 1);
        collect(x.right, prefix, queu);
    }
